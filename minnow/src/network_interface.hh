...
private:
  ...
  Address ip_address_;
  // mapping between IP address and ethernet address
  std::unordered_map<uint32_t,EthernetAddress> hashTmp;
  // Ethernet frames that are waiting to be sent
  std::deque<EthernetFrame> ready_ones;
  // mapping between <ip,mac> and the time when it was stored
  std::unordered_map<uint32_t, size_t> hashTime;
  // recording the IPs of the ready_ones (not including ARP messages)
  std::queue<uint32_t> ips;
  // meaning the time "now", useful in resend ARP requests and erase expired record in hashTmp
  size_t time;
  // function to push ethernet frame
